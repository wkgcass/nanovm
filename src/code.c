...
//    if (nvm_class->attributes_count > 0) {
//        for (int i = 0; i < nvm_class->attributes_count; i++) {
//            zfree(nvm_class->attributes[i].info);
//        }
//    }
//    if (nvm_class->methods_count > 0) {
//        for (int i = 0; i < nvm_class->methods_count; i++) {
//            if (nvm_class->methods[i].attrs) {
//                for (int j = 0; j < nvm_class->methods[i].attrs_count; j++) {
//                    zfree(nvm_class->methods[i].attrs[j].info);
//                }
//            }
//        }
//    }
//    if (nvm_class->fields_count > 0) {
//        for (int i = 0; i < nvm_class->fields_count; i++) {
//            zfree(nvm_class->fields[i].attrs)
//        }
//    }
}
...
int NanoVM_parse_code0(nvm_ctx_t* ctx, int bytecode_len, nvm_bytecode_t* bytecodes) {
    ...
    for (int i = 0; i < bytecode_len; i++) {
        ...
        int result = read_class(&bytecode, &nvm_class[i]);
        if (result == -1) {
...
int _create_prm_type(nvm_ctx_t* ctx, char prm_type) {
    ...
    ctx->vm->code_mgr->types[ctx->vm->code_mgr->type_len] = (nvm_type_t*) nvm_prm_type;
    ctx->vm->code_mgr->type_len++;
    if (!nvm_prm_type) {
